...
# To use a specific Java runtime, set an environment variable named HolyCubeRevolution_JAVA to the full path of java.exe.
# To disable automatic restarts, set an environment variable named HolyCubeRevolution_RESTART to false.
# To install the pack without starting the server, set an environment variable named HolyCubeRevolution_INSTALL_ONLY to true.
...
if ! command -v "${HolyCubeRevolution_JAVA:-java}" >/dev/null 2>&1; then
...
if [ "${HolyCubeRevolution_INSTALL_ONLY:-false}" = "true" ]; then
    echo "INSTALL_ONLY: complete"
    exit 0
fi

JAVA_VERSION=$("${HolyCubeRevolution_JAVA:-java}" -fullversion 2>&1 | awk -F '"' '/version/ {print $2}' | cut -d'.' -f1)
...
while true
do
    "${HolyCubeRevolution_JAVA:-java}" @user_jvm_args.txt @libraries/net/neoforged/neoforge/$NEOFORGE_VERSION/unix_args.txt nogui

    if [ "${HolyCubeRevolution_RESTART:-true}" = "false" ]; then
        exit 0
    fi
